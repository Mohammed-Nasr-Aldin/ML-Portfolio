(() => {
    ...
    function applyTheme(t) {
        ...
        if (themeMeta) themeMeta.setAttribute('content', t === 'dark' ? '#0D1526' : '#F5F7FA');
        readColors();
        if (reduce && W) draw(1);
    }
    ...
    /* ---------- Custom Navigation Handler for data-target (Prevents history pollution) ---------- */
    ...
    /* ---------- Active nav link mapping based on data-target ---------- */
    const links = $$('#nav-list a[data-target]');
    const linkMap = new Map(links.map(a => [a.getAttribute('data-target'), a]));
    ...
    if (viewerBox) {
        ...
        if (pdfInline && slidesFrame) {
            ...
            const fallback = $('.viewer-fallback', viewerBox);
